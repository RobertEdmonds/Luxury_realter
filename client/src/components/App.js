...
function App() {
  ...
  function handleShowHouse(house) {
    if (!customer && !employee) {
      ...
      fetch(`houses/${house.id}`).then((resp) => {
        if (resp.ok) {
    ...
    <div>
      ...
      <Route exact path="/sales">
        <Sales
          ...
          handleShowHouse={handleShowHouse}
          setToggle={setToggle}
          toggle={toggle}
        />
      </Route>
        ...
        <>
          <Route exact path={`/edit_photos/${houseId}`}>
            <EditPhoto
              houseId={houseId}
              images={houseImages}
              handleAddEditPhoto={handleAddEditPhoto}
          ...
          <Route exact path="/new_house">
            <NewHouseForm
              handleHouseForm={editing ? handleEditNewHouse : handleAddNewHouse}
              setOnMarket={setOnMarket}
              onMarket={onMarket}
              setUnderContract={setUnderContract}

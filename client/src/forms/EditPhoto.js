...
function EditPhoto({
  ...
  const [picture, setPicture] = useState(
    "https://freesvg.org/img/1410828243.png"
  );
  ...
  const displayPhotos = images
    .sort((a, b) => (a.order_number > b.order_number ? 1 : -1))
    .map((image) => {
      return (
        <img
          key={image.id}
          className="clickEditImage"
  ...
  function handleEditPhoto(e) {
    ...
    const findImage = images.filter(
      (image) => image.order_number === formData.order_number
    );
    ...
    const changeOldImage = images.map((image) => {
      if (findImage.length === 0) {
        ...
        return {
          id: findImage[0].id,
          picture_url: findImage[0].picture_url,
          order_number: newImage[0].order_number,
        };
      } else {
        return image;
      }
